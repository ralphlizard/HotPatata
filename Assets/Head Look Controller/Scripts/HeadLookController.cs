...
public class BendingSegment {
	...
	public float maxVerAngle = 80;
	public float responsiveness = 5;
	internal float angleH;
	internal float angleV;
	internal float deltaH;
...
public class HeadLookController : MonoBehaviour {
	...
	void LateUpdate () {
		...
		foreach (BendingSegment segment in segments) {
			...
			// original angle minus multiplied angle
			float deltaH = hAngle;
			float oAngle = hAngle;

			// Handle max bending angle here
			hAngle = Mathf.Clamp(hAngle, -segment.maxHorAngle, segment.maxHorAngle);
			...
			if (vAngle > -segment.maxVerAngle && vAngle < segment.maxVerAngle)
			{
				segment.deltaH = Mathf.Lerp(
					segment.deltaH, deltaH, Time.deltaTime * segment.responsiveness
				);
